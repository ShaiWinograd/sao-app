...
export async function renderCustomerReportPdf(model: CustomerReportPdfModel): Promise<Buffer> {
  ...
  // fontkit's bidi reorders RTL runs correctly but drops the neutral space next
  // to Hebrew ("דוח לקוח" → "לקוחדוח", "לקוח: דנה" → "לקוחדנה"). Convert a space to a
  // non-breaking space whenever it touches Hebrew (directly or across
  // punctuation) but NOT when it touches a digit — so numeric/date/amount runs
  // keep their own left-to-right order and their digits are never reversed.
